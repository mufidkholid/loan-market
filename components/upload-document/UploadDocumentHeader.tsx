...
const UploadDocumentHeader = () => {
  ...
  return (
    <div>
      ...
      {/* Baris atas: Tombol panah + teks */}
      <div className="flex flex-col sm:flex-row items-center justify-between w-full mt-6 mb-6 space-y-2 sm:space-y-0">
        <button onClick={() => router.push("/loan/address")} className="flex items-center justify-center w-9 h-9 bg-[#17A9E2] text-white rounded-full hover:bg-[#1495c9] transition-colors">
          <FiArrowLeft size={20} />
        </button>

        <div className="text-center flex flex-col items-center px-2">
          <h1 className="text-lg sm:text-xl font-bold text-gray-800">Upload Dokumen</h1>
          <p className="text-xs sm:text-sm text-gray-600">Silahkan upload dokumen yang dibutuhkan</p>
        </div>

        <button onClick={() => router.push("/loan/choose-product")} className="flex items-center justify-center w-9 h-9 bg-[#17A9E2] text-white rounded-full hover:bg-[#1495c9] transition-colors">
